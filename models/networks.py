...
class NonLinearRegres(nn.Module):
    """ Simple linear regressor """
    def __init__(self, ROIs, ampli=2):
        super().__init__()
        features = ROIs*(ROIs-1)//2 # Lower triangular connections
        self.multi_layer = nn.Sequential(
            nn.Linear(in_features=features, out_features=features*ampli, dtype=torch.float64),
            nn.Sigmoid(),
            nn.Linear(in_features=features*ampli, out_features=features, dtype=torch.float64)
        )
