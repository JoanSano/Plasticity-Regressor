...
from .paths import get_info, get_subjects
...
def create_anat_prior(data_path, rois=170, threshold=.2, save=False):
    """ Creates a prior distribution of connections between rois """
    files = get_subjects(data_path, session='control')
    control = np.zeros((len(files), rois*(rois-1)//2))
    patients = list()
    for i, f in enumerate(files):
        path, session, patient, name = get_info(f)
        patients.append(patient)
        graph = np.log1p(GraphFromCSV(path+name+'.csv', name, rois=rois).get_connections())
        control[i] = graph
    ...
    if save:
        np.savetxt(data_path+'prior.csv', prior.view(1, -1), delimiter=',')
        np.savetxt(data_path+'mean_conn.csv', mean_connections.view(1, -1), delimiter=',')
